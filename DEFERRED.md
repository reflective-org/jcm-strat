...
| Item | Label | Notes |
|---|---|---|
| QBO amplitude vs tau: **settled** — sweep 10 / 5 / 2 / 1 d = 80 / 86 / 90 / 92 % of ERA5 over 2005-2009 (`p8b/c/d/e_5yr`), nothing else moving; tau 1 d is the default (KEY_DECISIONS #27). The last ~8 % is the target shape, addressed by the mean-preserving interpolation (`p8f_*`). Still open: whether the recurring February 2006 vortex reversal at short tau (never at 10 d) is a Holton-Tan-type response or variability; needs more than three chains. | research | tau 6 h not run: at 1 d the gain per halving is already ~2 %. |
