...
| Item | Label | Notes |
|---|---|---|
| [#51](https://github.com/reflective-org/jcm-strat/issues/51) Fast mode: dt 30 min on strat63 for sensitivity work (Phase 7's pending decision), stated with its 18 % weaker Antarctic vortex | research | The only route to ~30 simulated years per hour together with #19 (output overhead) and #26 (streamed target); one 2005-2009 chain `p9_l63 + run.time_step=30` against `p9_t63l63_5yr`. |
| [#47](https://github.com/reflective-org/jcm-strat/issues/47) JCM truncates `save_interval / time_step` and `total_time / save_interval` silently (`model.py`), and the chunk loop labels a tail chunk shorter than a save without integrating it (`runners.py`), so a 7-min step or a 91-day segment loses time with no error; the 366-day 2008 segments of every chain so far integrated 365 days | bug | `jcm_strat/segments.py` and `chain_segments.sh` refuse both cases; upstream should raise. Upstream issue to file. |
| [#48](https://github.com/reflective-org/jcm-strat/issues/48) ERA5 prefetch (`jcm.data.era5.dataset_on_model_grid`) is single-threaded and holds ~5-7x the output window in RAM (a 20 GB T63L63 year peaks near 140 GB); the whole Phase 9 matrix (2 TB) takes ~10 h in three parallel streams and is CPU-, not network-bound | perf | Chunk the window in time inside `_to_model_grid`, or regrid per level in float32. Upstream issue to file. |
| [#49](https://github.com/reflective-org/jcm-strat/issues/49) Native terrain exists only for T63 and T106; T85 and T119 run on the interpolated T63 file (fine for the dry model, wrong once Lott-Miller SSO is on) | feature | The GMTED2010 builder in `jcm/data/mirror` runs on Glade; add t85 / t119 bundles there. |
| [#50](https://github.com/reflective-org/jcm-strat/issues/50) `tracer_budget.py`'s analytic sai target depends on the vertical table: the source-box mass fraction comes out 0.00811 on L95/strat63 and 0.00768 on strat47 (the model burdens agree to 0.5 % across the three, unity error 3e-4), so strat47 reads "+3.5 % vs analytic" where L95 reads -0.8 % | bug | The budget masks the box on nominal sigma x p_s per column while the model masks on a + b p_s (identical for pure-pressure layers); find the layer that flips between the two and mask on the file's own `hybrid_a_full/hybrid_b_full` instead. Diagnostic only. |

## Found in Phase 10

| Item | Label | Notes |
|---|---|---|
| End-to-end throughput is output-bound at 6-hourly instantaneous output: 13 s of stepping per 10-day chunk against ~17 s of conversion and netCDF writing (`preds.to_xarray()` + uncompressed `to_netcdf`, issue #19). A 30-year run is ~15 h instead of ~7. Options: write the chunk asynchronously (a second thread/process while the next chunk steps), write float16 or scaled int16 for the tracers, or drop the once-only pulses after they have mixed out. | feature | Phase 10 review run: stepping 4,760 d/hr, end-to-end 860. |
| Omega in snapshot mode is the diagnostic computed at the start of the last inner step, one 12-min step behind u, v, T. | note | Document for the ML users; a fix needs JCM's snapshot path to publish the physics carry of the same step. |
