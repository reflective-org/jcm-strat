...
#
# Restartable: a finished segment (exit=0 in its log) is skipped. Progress: runs/p4_chain.log
set -uo pipefail
REPO="$(cd "$(dirname "${BASH_SOURCE[0]}")/.." && pwd)"; cd "$REPO"
# shellcheck disable=SC1091
source "$REPO/scripts/env.sh"
LOG="$REPO/runs/p4_chain.log"; mkdir -p "$REPO/runs"
...
prev=""
for y in "${YEARS[@]}"; do
  name="p4_$y"; rundir="$REPO/runs/$name"
  ...
  ( cd "$rundir" && python -m jcm_strat.main --config-dir "$REPO/jcm_strat/config" +experiment=p4_5yr \
...
agg="$REPO/runs/p4_5yr"; mkdir -p "$agg"; rm -f "$agg"/longrun_day*.nc
offset=0
for y in "${YEARS[@]}"; do
  for f in "$REPO/runs/p4_$y"/longrun_day*.nc; do
...
cp "$REPO/runs/p4_2005/.hydra" -r "$agg/" 2>/dev/null
cat "$REPO"/runs/p4_20??/log.txt > "$agg/log.txt"
